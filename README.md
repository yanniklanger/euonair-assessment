...
**Wichtig:** Verwenden Sie ausschließlich Schnittstellen (APIs) oder Zugriffe, die öffentlich und lizenzkonform sind. 
Achten Sie auf mögliche Rate Limits und Nutzungsbedingungen (Terms of Service, TOS)

#### Suchfunktion
- Ermöglichen Sie mindestens die logische Verknüpfung von Suchbegriffen per AND (Pflicht) und optional OR und NOT.
...
Fassen Sie alle analysierten Informationen in einem Report zusammen (CSV, JSON, PDF, Markdown etc.).
Geben Sie sowohl die Suchparameter als auch kurze statistische Übersichten aus, z. B. Anzahl gefundener Publikationen, Publikationen pro Jahr oder häufigste Schlüsselbegriffe.

#### Optionaler Kurzbericht
Erstellen Sie (wenn möglich) eine Kurzfassung (z. B. 1–2 Seiten PDF oder Markdown), in der Sie:
- Ihre Suchstrategie erläutern
- Die gewählte(n) KI-Methode(n) beschreiben
- Herausforderungen und mögliche Erweiterungen ansprechen

#### Nachvollziehbarkeit
Sorgen Sie dafür, dass Ihre Ergebnisse mit einem knappen Beispiel (z. B. 3–5 Publikationen) reproduzierbar sind.


## 2. Technische Anforderungen
- Programmiersprache: Python 3.8 oder höher
- Python-Bibliotheken: frei wählbar (z. B. requests, BeautifulSoup, pandas, spacy, transformers, openai, etc.)
- Modularer Code:
...
- Requirements/Dependency Management: Stellen Sie eine requirements.txt, Pipfile oder pyproject.toml bereit.
- Versionskontrolle:
